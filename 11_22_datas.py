...
class Portfolio(object):  # 个体
    ...
    def __init__(self):
        ...
        self._locked_cash = 0
        p = [Position(self.context.universe())]
        self._positions = Series(p, index=self.context.universe())
        # 11_22#
        # 删除了两个属性
        # 11_22#
...
# 11_22#
    def Change(self,order):
        ...
        if(order.is_buy==True): # 买入
            ...
            if(flg!=0):
                return -1
            self._available_cash -= money * (1 + Config.counter_fee())
            self._cash -= money * (1 + Config.counter_fee())
        else: # 卖出
            flg = self._positions[order.stock_name].Change(order)
            if(flg!=0):
                return -1
            self._available_cash += money * (1 - Config.counter_fee()+Config.stamp_duty())
            self._cash += money * (1 - Config.counter_fee()+Config.stamp_duty())
        return 0
...
class Order(object):
    def __init__(self, amount, stock_name, price, is_buy=True):
        self.is_buy = is_buy
        self.amount = amount
...
class OrderList(object):
    ...
    def order(self, stock_name, amount, style=None):  # return id
        ...
        if amount + position.total_amount < 0:
            ...
            if amount >= 0:
                o = Order(amount, stock_name, self.get_current_price(stock_name))  # port
            else:
                o = Order(-amount, stock_name, self.get_current_price(stock_name), False)  # port
            self.List[self.i] = o
    ...
    def order_target(self, stock_name, amount, style=None):  # return id
        ...
        if amount < 0:
            ...
            if gap >= 0:
                o = Order(gap, stock_name, self.get_current_price(stock_name))  # port
            else:
                o = Order(-gap, stock_name, self.get_current_price(stock_name), False)  # port
            self.List[self.i] = o
            self.Backup[self.i] = o
            self.i += 1
            return (self.i - 1)
    ...
    def order_value(self, stock_name, value, style=None):  # return id
        ...
        if value + position.total_value < 0:
            ...
            if value >= 0:
                o = Order(value / price, stock_name, self.get_current_price(stock_name))  # port
            else:
                o = Order(-value / price, stock_name, self.get_current_price(stock_name), False)  # port
            self.List[self.i] = o
            self.Backup[self.i] = o
            self.i += 1
            return (self.i - 1)
    ...
    def order_target_value(self, stock_name, value, style=None):  # return id
        ...
        if value < 0:
            ...
            if gap >= 0:
                o = Order(gap / price, stock_name, self.get_current_price(stock_name))  # port
            else:
                o = Order(-gap / price, stock_name, self.get_current_price(stock_name), False)  # port
            self.List[self.i] = o
            self.Backup[self.i] = o
            self.i += 1
            return (self.i - 1)
