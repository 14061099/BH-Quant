...
class Context(object):  # 全局变量
    ...
    def __init__(self,start_date,end_date):
        ...
        self._current_dt = datetime.strptime(start_date,'%Y.%m.%d.%H.%M')  # 当前时间
        self._previous_dt = None
        self._previous_dt = None  # 前一个时间
        self._start_date = datetime.strptime(start_date,'%Y.%m.%d.%H.%M')   # 回测起时
        self._end_date = datetime.strptime(end_date,'%Y.%m.%d.%H.%M')   # 回测终时
        self._style = 'huice'  # ？？？
...
class OrderList(object):
    ...
    def get_current_price(self, stock_name):
        return self.security.security[self.context.current_dt]['end']
